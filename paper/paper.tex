...
\section{Methodology}
We propose a method with different variants to model the dependencies between answers, by feeding answer pairs jointly to BERT's architecture. by doing so, we contribute in different ways:
...
% TODO: add a diagram displaying our construct and the score calculation (including calculating for (i,j) and (j,i) and averaging with 1-s_(j,i)).
...
In the first part, we trained solely on the dataset alterations part, without intergrating the pair scores. That is, our model was trained directly on identifying the better answer in a pair
